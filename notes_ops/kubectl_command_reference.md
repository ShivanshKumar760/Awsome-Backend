...
This is the single most useful trick for *learning* manifest structure, or for quickly scaffolding a YAML file you'll then hand-edit, instead of writing one from scratch.
...
- **`Events` (bottom section)** — the actual timeline of what the scheduler/kubelet did and why, including pull errors, OOMKills, failed mounts, failed probes. This is where you find the *real* root cause, not just the symptom.
...
This is exactly the `apply`-driven workflow used throughout the earlier Instagram API setup — and `apply`'s "patch, don't fully overwrite" behavior is also precisely why the PVC resize attempt failed earlier: it tried to *patch* an immutable field instead of erroring out as cleanly as `replace` would have.
...
**Why `rollout history` entries appear at all:** every time you change a Deployment's Pod template (image, env vars, etc.), Kubernetes creates a **new ReplicaSet** under the hood (as covered in the k8s concepts notes) and records it as a new revision — `rollout undo` works by scaling the *previous* ReplicaSet back up and the current one down, not by "undoing" individual field changes.
...
**Why this matters with `imagePullPolicy: IfNotPresent`** (as in your Deployment manifest): that setting means the kubelet will happily keep using whatever image with that exact tag is *already present* on the node, even if you've rebuilt a "better" one with the same tag on your host and reloaded it — `crictl images` is how you confirm whether the node's copy is actually current, and `crictl rmi` plus a fresh `kind load` is the guaranteed way to eliminate any ambiguity, rather than guessing whether your latest fix actually made it into the running container.
...
```
**Use case:** isolating a multi-replica bug requires *actually* getting down to 1 replica — deleting the HPA (not just scaling) is what makes that stick, since the HPA would otherwise immediately undo a plain `kubectl scale`.
...
```
**Use case:** tailing logs across *every* Pod in a Deployment at once, with `--prefix` so each line is labeled by source Pod — `--max-log-requests` raises the default cap of 5 concurrent streams, needed once you're running more than 5 replicas (we hit this exact error mid-session).
...
```
**Use case:** shows the *exact* command line PID 1 (the container's main process) was actually started with — the definitive way to confirm what `CMD`/`ENTRYPOINT` really resolved to at runtime, useful when a Dockerfile looks right but you want to rule out a stale image or an overridden command.
